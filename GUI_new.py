...
class LinearCodeGUI(tk.Tk):
    ...
    def _build_main_layout(self):
        ...
        ttk.Label(code_frame, text="Code:", style="Header.TLabel").grid(row=0, column=0, padx=(0, 5), sticky="w")
    ...
    def _on_code_changed(self):
        ...
        try:
            ...
            # Remove '(k_tag)' from label – show numeric value instead
    ...
    def _build_batch_tab(self):
        ...
        # --- Summary ---
        ttk.Label(summary_frame, text="Batch Summary Metrics", style="Section.TLabel").pack(
            anchor="w", pady=(0, 5)
        )
    ...
    def _build_grid_tab(self):
        ...
        ttk.Label(
            params_frame,
            text=(
                "Channel error probabilities p "
                "(comma-separated or [start:step:end], 0–1):"
            ),
        ...
        ttk.Entry(params_frame, textvariable=self.grid_p_list_var, width=40).grid(
            row=1, column=1, sticky="we", padx=(5, 25)
        )

        ttk.Label(
            params_frame,
            text=(
                "Deliberate errors list "
                "(comma-separated or [start:step:end]):"
            ),
        ...
        ttk.Entry(params_frame, textvariable=self.grid_delib_list_var, width=40).grid(
            row=2, column=1, sticky="we", padx=(5, 25)
        )
        ...
        self.grid_x_combo.grid(row=1, column=3, sticky="w")
        ...
        self.grid_y_combo.grid(row=2, column=3, sticky="w")
        ...
        run_btn.grid(row=3, column=2, rowspan=2, padx=(10, 0), pady=(0, 0), sticky="nswe")

        clear_btn = ttk.Button(
            params_frame,
            text="Clear",
            command=self._clear_grid_tab,
        )
        clear_btn.grid(row=3, column=3, rowspan=2, padx=(10, 0), pady=(0, 0), sticky="nswe")
        ...
        # Text area
        header_frame = ttk.Frame(left, style="Card.TFrame")
        header_frame.pack(side=tk.TOP, fill=tk.X)
        ttk.Label(header_frame, text="Grid Results (per grid point)", style="Section.TLabel").pack(
            side=tk.LEFT, anchor="w", pady=(0, 5)
        )

        export_grid_btn = ttk.Button(
            header_frame,
            text="Export grid to CSV",
            command=self._export_grid_results_to_csv,
        )
        export_grid_btn.pack(side=tk.RIGHT, padx=(0, 5), pady=(0, 5))
        ...
        # React to X/Y changes
        self.grid_x_combo.bind("<<ComboboxSelected>>", lambda e: self._plot_grid_results(self.grid_last_results))
        self.grid_y_combo.bind("<<ComboboxSelected>>", lambda e: self._plot_grid_results(self.grid_last_results))
    ...
    def _plot_grid_results(self, results: list[dict]):
        ...
        # Fix legend warning: only call legend if we actually plotted something
        if groups:
            self.grid_ax.legend()
    ...
    def _clear_grid_tab(self):
        ...
        if HAS_MPL and self.grid_ax and self.grid_canvas:
            self.grid_ax.clear()
            self.grid_canvas.draw()
